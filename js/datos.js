...
/* ── DATOS DEMO ── */
var PROPS_DEMO = [
  {id:'d1',titulo:'Villa Las Palmas',   tipo:'Villa',       precio:25000, ciudad:'Punta Cana',   sector:'La Altagracia', dist:'1.8 km',hab:3,ban:2,m2:120,emoji:'🏠',grad:'linear-gradient(135deg,#FDEBC8,#F0B86E)', top:true, nuevo:false,wa:'18090000001',val:4.9,res:34,foto:'',desc:'Hermosa villa con piscina privada y vista al mar. Amueblada, todos los servicios incluidos.',comod:['📶 WiFi','❄️ A/C','🌊 Vista mar','🐾 Mascotas OK','🍳 Cocina equipada','⚡ Planta eléc.','🏊 Piscina','🔐 Seguridad 24h'],prop:'Roberto Vargas',opiniones:[{n:'Ana López',i:'AL',c:'#1B4F8A',s:'★★★★★',t:'¡100% recomendada!'},{n:'Carlos Marte',i:'CM',c:'#10B981',s:'★★★★☆',t:'Muy cómoda, planta siempre funciona.'}]},
  {id:'d2',titulo:'Apto. Vista Mar',    tipo:'Apartamento', precio:18000, ciudad:'Santo Domingo', sector:'Naco',          dist:'3.2 km',hab:2,ban:1,m2:75, emoji:'🌴',grad:'linear-gradient(135deg,#C8E6D4,#7ABFA0)', top:false,nuevo:true, wa:'18090000002',val:4.8,res:21,foto:'',desc:'Moderno apartamento en zona exclusiva. Cerca de todo.',comod:['📶 WiFi','❄️ A/C','🚗 Parqueo'],prop:'Luisa Castro',opiniones:[{n:'Pedro M.',i:'PM',c:'#6B3FA0',s:'★★★★★',t:'Excelente ubicación.'}]},
  {id:'d3',titulo:'Cabaña El Río',      tipo:'Cabaña',      precio:12000, ciudad:'Jarabacoa',     sector:'Confluencia',   dist:'45 km', hab:2,ban:1,m2:60, emoji:'🌊',grad:'linear-gradient(135deg,#C8DFFB,#7AAEE0)', top:false,nuevo:false,wa:'18090000003',val:4.7,res:18,foto:'',desc:'Cabaña rodeada de naturaleza con vista al río.',comod:['📶 WiFi','🌿 Jardín','🌊 Vista río'],prop:'Juan Díaz',opiniones:[{n:'María G.',i:'MG',c:'#E85D3A',s:'★★★★☆',t:'Lugar espectacular.'}]},
  {id:'d4',titulo:'Casa Familiar',      tipo:'Casa',        precio:22000, ciudad:'Santiago',       sector:'Los Jardines',  dist:'5.1 km',hab:4,ban:2,m2:140,emoji:'🏘️',grad:'linear-gradient(135deg,#FBC8C8,#E07A7A)', top:false,nuevo:true, wa:'18090000004',val:4.6,res:15,foto:'',desc:'Espaciosa casa familiar con patio grande.',comod:['📶 WiFi','❄️ A/C','🚗 Parqueo','🌿 Jardín'],prop:'Ana López',opiniones:[{n:'Carlos R.',i:'CR',c:'#1B4F8A',s:'★★★★☆',t:'Propietaria muy atenta.'}]},
...
function previsualizarFotos(input) {
  ...
  Array.from(input.files || []).slice(0, 5).forEach(function(f) {
    ...
    r.onload = function(e) {
      ...
      d.style.cssText = 'width:72px;height:72px;border-radius:10px;background-image:url(' + e.target.result + ');background-size:cover;background-position:center;border:2px solid #E2EAF4;flex-shrink:0';
      prev.appendChild(d);
    };
    r.readAsDataURL(f);
  });
...
function gradTipo(t) {
  return {Casa:'linear-gradient(135deg,#FDEBC8,#F0B86E)',Apartamento:'linear-gradient(135deg,#C8E6D4,#7ABFA0)',Villa:'linear-gradient(135deg,#E8C8FB,#B07AE0)',Cabaña:'linear-gradient(135deg,#C8DFFB,#7AAEE0)',Estudio:'linear-gradient(135deg,#FBC8C8,#E07A7A)'}[t] || 'linear-gradient(135deg,#FDEBC8,#F0B86E)';
}
function esNuevo(ts) {
  if (!ts) return false;
